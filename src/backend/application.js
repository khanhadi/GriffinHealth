...
class RAGVectorStore {
    ...
    async addHealthDocument(data, date) {
        try {
            ...
            this.documents = this.documents.filter(doc => new Date(doc.date) > sixMonthsAgo);
...
const specialistAgents = {
    ...
    SLEEP: {
        ...
        metrics: ['awake_duration', 'core_duration', 'deep_duration', 'rem_duration']
    },
    ACTIVITY: {
...
class SwarmCoordinator {
    ...
    async runAgentAnalysis(specialty, agent) {
        ...
        return { specialty, analysis: response.choices[0].message.content, statistics: calculatedStats };
    }
    ...
    calculateStatistics(metricsData) {
        ...
        for (const [metric, records] of Object.entries(metricsData)) {
            const values = records.map(record => parseFloat(record.value)).filter(val => !isNaN(val));
            if (values.length > 0) {
                stats[metric] = {
    ...
    extractComments(text) {
        const insightsMatch = text.match(/(recommend|suggest|advis(e|ing)|should)\s[^.,]*[.,]/gi);
        const comments = insightsMatch ? insightsMatch.map(comment => comment.trim()) : ["No specific comments"];
        return comments;
    }
    ...
    return new Promise((resolve, reject) => {
        fs.createReadStream(filePath)
            ...
            .on('data', (row) => {
                ...
                const durationMinutes = end.isValid && start.isValid ? end.diff(start, 'minutes').minutes : 0;
                ...
                switch (type) {
                    ...
                    case 'RespiratoryRate':
                        data.respiratory_rate.push({ value: parseFloat(value), startDate, endDate });
                        break;
                    case 'EnvironmentalAudioExposure':
                        data.audio_exposure.push({ value: parseFloat(value), startDate, endDate });
                        break;
                    default:
                        break;
                }
